package com.example.charlesanderson.oddsare.RoundKnob;
...
public class KnobSingleton extends Application {
    private final static String					LOG_TAG				= "KnobSingleton";
    private static KnobSingleton m_Instance;
    ...
    public KnobSingleton() {
        super();
        m_Instance = this;
    }

    public static KnobSingleton getInstance() {
        if(m_Instance == null) {
            synchronized(KnobSingleton.class) {
                if(m_Instance == null) new KnobSingleton();
            }
        }
        return m_Instance;
    }
    ...
    //must be called in every oncreate for the knob
